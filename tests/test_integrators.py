# -*- coding: utf-8 -*-
"""Test all integrators with a PythonKernel-based cosine ODE."""
...
# ---------------------------------------------------------------------------
# Cosine kernel (PythonKernel)
# ---------------------------------------------------------------------------

class CosineKernel(PythonKernel):
    """w'_j = cos(phi0_j + omega*t), known analytical solution available."""
    ...
    def callback(self, t):
        for j in range(self.n):
            phi0_j = (float(j + 1) / self.n) * 2.0 * np.pi
            self.out[j] = np.cos(phi0_j + self.omega * t)

    def reference_solution(self, tt):
        tt = np.atleast_1d(tt)
        ww = np.empty((tt.shape[0], self.n), dtype=DTYPE, order="C")
        sol = lambda t, phi0: 1.0 / self.omega * np.sin(phi0 + self.omega * t)
        for j in range(self.n):
            phi0_j = (float(j + 1) / self.n) * 2.0 * np.pi
            ww[:, j] = sol(tt, phi0_j) - sol(0.0, phi0_j)
        return ww
...
def test_integrator_python_kernel(integrator, rel_tol, is_galerkin):
    nt_vis = 1
    if is_galerkin:
    ...
    rhs = CosineKernel(N, omega=OMEGA)

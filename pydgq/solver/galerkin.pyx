...
from pydgq.data import load_data as _load_data_file, data_file_path as _data_file_path
...
"""
        ...
        if datamanager is None:
            raise RuntimeError(f"{name}: init() must be called first.")
        if not datamanager.available:
            raise RuntimeError(f"{name}: Cannot use Galerkin integrators because the auxiliary class did not initialize.")
        if datamanager.method != name:
            raise RuntimeError(f"{name}: Trying to integrate with {name}, but the auxiliary class has been initialized for {datamanager.method}.")
...
"""
        ...
        if nt_vis < 1:
            raise ValueError( f"nt_vis must be >= 1; got {nt_vis}" )
        if method not in ["dG", "cG"]:
            raise ValueError( f"Unknown method '{method}'; valid: 'dG', 'cG'" )
    ...
    def __load_data(self, rule):
        """Load precalculated data (pydgq_data.npz).
...
"""
        ...
        #
        if q > maxq:
            raise ValueError(f"Data file '{data_path}' contains data up to degree maxq={maxq}, but the requested degree was higher, q={q}. Use a lower q or re-run pydgq.utils.precalc for higher maxq.")
        ...
        if need_nt_vis > maxnt_vis:
            raise ValueError(f"Data file '{data_path}' contains data up to visualization maxnt_vis={maxnt_vis}, but the needed number of points is higher, {need_nt_vis}. Use a lower nt_vis or re-run pydgq.utils.precalc for higher maxnx (same thing as our maxnt_vis).")
        if rule is not None and rule > maxrule:
            raise ValueError(f"Data file '{data_path}' contains data for integration rules up to order {maxrule}, but the requested order was higher, rule={rule}. Use a lower rule.")
